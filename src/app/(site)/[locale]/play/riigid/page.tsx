...
import { Riigid } from "@/components/Riigid";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Play.riigid" });

  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    alternates: {
      canonical: locale === "en" ? "/play/riigid" : `/${locale}/play/riigid`,
      languages: {
        en: "/play/riigid",
        et: "/et/play/riigid",
      },
...
export default async function RiigidPage({ params }: PageProps) {
  const { locale } = await params;
